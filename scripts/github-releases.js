...
program.command('upload-asset <tag> <asset-path>').action(
  wrapCommand(async (tag, assetPath) => {
...
program.command('create-draft <tag> <release-notes-path>').action(
  wrapCommand(async (tag, releaseNotesPath) => {
...
program.command('publish <tag>').action(
  wrapCommand(async tag => {
    // Request all releases, as we cannot request by tag name for draft releases
    const { data: releases } = await octokit.repos.listReleases({
      owner: GITHUB_OWNER,
      repo: GITHUB_REPO,
