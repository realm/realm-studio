          ...
          script {
            if (PUBLISH == 'true') {
              ...
              if (env.PREPARE == 'true') {
                ...
                def baseName = java.net.URLDecoder.decode(env.JOB_BASE_NAME).replaceAll("[^A-Za-z0-9\\-\\_]", "-")
                env.NEXT_VERSION = sh(
                  script: "npm version ${NEXT_VERSION}-${baseName}.${BUILD_NUMBER} --no-git-tag-version",
                  returnStdout: true,
                ).trim()
                // Set the build display name
        ...
        nvm(env.NODE_VERSION) {
          ...
          withCredentials([
            ...
            script {
              ...
              // Create a draft release on GitHub
              def prId = sh(
                script: "node scripts/github-releases create-pull-request ${PREPARED_BRANCH} ${BRANCH_NAME} 'Prepare version ${NEXT_VERSION}' --assignee ${assignee} --reviewer bmunkholm --print-number",
                returnStdout: true,
              ).trim()
